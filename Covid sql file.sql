...
FROM state_wise
WHERE State_Code="WB"
group by State_Code="WB";

SELECT State_Code,SUM(COUNT) as Tested from district_wise
WHERE State_Code= "WB"
GROUP BY State_Code;


SELECT State_Code,SUM(COUNT) as Tested from state_wise
WHERE State_Code= "WB"
GROUP BY State_Code;
...
SELECT State_Code,SUM(Count) as State_Population
FROM district_wise
...
sum(First_Dose) as "First Does",sum(Second_Dose) as "Second Dose"
FROM
(
SELECT YEAR(DATE) AS Year,MONTH(DATE) as Month_num,MONTHNAME(DATE) as Month,DAY(Date) as Day,
SUM(CASE WHEN Status='tested' THEN Count ELSE '0' END) as Tested,
...
GROUP BY Year,Month,week_of_month,Month_num
ORDER BY Year,Month_num,week_of_month;
...
SELECT s.state_name,
SUM(CASE WHEN d.Status='population' THEN d.Count ELSE '0' END) as Population,
SUM(CASE WHEN s.Status='confirmed' THEN s.Count ELSE '0' END) as Confirmed,
SUM(CASE WHEN s.Status='deceased' THEN s.Count ELSE '0' END) as Deceased,
SUM(CASE WHEN s.Status='tested' THEN s.Count ELSE '0' END) as tested,
SUM(CASE WHEN d.Status='vaccinated1' THEN d.Count ELSE '0' END) as First_Dose,
SUM(CASE WHEN d.Status='vaccinated2' THEN d.Count ELSE '0' END) as Second_Dose
FROM  state_wise s
INNER JOIN district_wise d
